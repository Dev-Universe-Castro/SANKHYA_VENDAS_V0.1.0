...
export const usersService = {
  ...
  async ensureSuperAdmin(): Promise<void> {
    const SUPER_EMAIL = 'sup@sankhya.com.br';
    const SUPER_PASSWORD = 'SUP321';

    try {
      const existingUsers = await this.search(SUPER_EMAIL);
      
      if (existingUsers.length === 0) {
        console.log('🔧 Criando usuário superadmin...');
        const hashedPassword = await cryptoService.hashPassword(SUPER_PASSWORD);
      ...
      console.error('❌ Erro ao verificar/criar superadmin:', erro.message);
    }
  }
};
